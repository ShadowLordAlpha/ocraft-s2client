...
public abstract class CameraModule {
    ...
    public static final float moveFactor = 1f;
    ...
    protected void moveCameraIsAttacking() {
        ...
        for (UnitInPool unit : m_client.observation().getUnits()) {
    ...
    protected void moveCameraScoutWorker() {
        ...
        for (UnitInPool unit: m_client.observation().getUnits(u -> isWorkerType(u.unit().getType()))) {
    ...
    protected void moveCameraNukeDetect() {
        ...
        for (EffectLocations effects: m_client.observation().getEffects()) {
    ...
    protected void moveCameraArmy() {
        ...
        for (UnitInPool unit : m_client.observation().getUnits()) {
            ...
            int nrUnitsNearby = 0;
            for (UnitInPool nearbyUnit: m_client.observation().getUnits()) {
    ...
    protected void updateCameraPosition() {
        if (cameraFocusUnit != null && followUnit && isValidPos(cameraFocusUnit.unit().getPosition().toPoint2d())) {
    ...
    protected boolean isValidPos(Point2d pos) {
        ...
        Size2dI mapSize = m_client.observation().getGameInfo().getStartRaw().get().getMapSize();
    ...
    protected boolean isAttacking(Unit attacker) {
        ...
        Set<Weapon> weapons = m_client.observation().getUnitTypeData(false).get(attacker.getType()).getWeapons();
    ...
    protected void setPlayerStartLocations() {
        ...
        List<UnitInPool> bases = m_client.observation().getUnits(u -> FILTER_TWONCENTER.contains(u.unit().getType()));
    ...
    protected void setPlayerIds() {
        ...
        m_playerIDs.clear();
        for (PlayerInfo player: m_client.observation().getGameInfo().getPlayersInfo()) {
